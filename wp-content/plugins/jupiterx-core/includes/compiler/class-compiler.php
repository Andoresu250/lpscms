...
final class _JupiterX_Compiler {
	...
	private function kill() {
		...
		$html = jupiterx_output( 'jupiterx_compiler_error_title_text', sprintf(
			'<h2>%s</h2>',
			__( 'Not cool, Jupiter cannot work its magic :(', 'jupiterx' )
		) );

		$html .= jupiterx_output( 'jupiterx_compiler_error_message_text', sprintf(
			'<p>%s</p>',
			__( 'Your current install or file permission prevents Jupiter from working its magic. Please get in touch with Jupiter support. We will gladly get you started within 24 - 48 hours (working days).', 'jupiterx' )
		) );
		...
		$html .= jupiterx_output( 'jupiterx_compiler_error_contact_text', sprintf(
			...
			__( 'Contact Jupiter Support', 'jupiterx' )
		) );
		...
		$html .= jupiterx_output( 'jupiterx_compiler_error_report_text', sprintf(
			...
			__( 'Send us an automatic report', 'jupiterx' ),
			__( 'We respect your time and understand you might not be able to contact us.', 'jupiterx' )
		) );
	...
	private function report() {
		...
		$message = jupiterx_output(
			...
			sprintf(
				'<p>%s<p>',
				__( 'Thanks for your contribution by reporting this issue. We hope to hear from you again.', 'jupiterx' )
			)
		);
