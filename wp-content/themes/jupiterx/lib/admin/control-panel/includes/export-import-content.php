...
class JupiterX_Control_Panel_Export_Import {
	...
	public function ajax_handler() {
		...
		if ( empty( $step ) ) {
			wp_send_json_error(
				__( 'Step param is missing.', 'jupiterx' )
			);
		}
		...
		if ( 'Import' === $type ) {
			...
			if ( empty( $attachment_id ) ) {
				wp_send_json_error(
					__( 'Attachment ID param is missing.', 'jupiterx' )
				);
			}
	...
	private function export( $step ) {
		...
		wp_send_json_error(
			sprintf( __( 'Step param (%s) is not valid.', 'jupiterx' ), $step )
		);
	}
	...
	private function export_start() {
		try {
			...
			throw new Exception( __( 'A problem occurred in cleaning export directory.', 'jupiterx' ) );
		} catch ( Exception $e ) {
	...
	private function export_content() {
		try {
			...
			if ( ! $this->jupiterx_filesystem->put_contents( $file_path, $content ) ) {
				throw new Exception( __( 'A problem occurred in exporting Content.', 'jupiterx' ) );
			}
	...
	private function export_widgets() {
		try {
			...
			throw new Exception( __( 'A problem occurred in exporting widgets.', 'jupiterx' ) );
		} catch ( Exception $e ) {
			return wp_send_json_error( $e->getMessage() );
		} // End try().
	}
	...
	private function export_settings() {
		try {
			...
			if ( ! is_array( $data ) ) {
				throw new Exception( __( 'All settings in Settings are set to default. Uncheck the Settings option or change one setting in Settings then export.', 'jupiterx' ) );
			}

			if ( ! $this->jupiterx_filesystem->put_contents( $file_path, wp_json_encode( $data ) ) ) {
				throw new Exception( __( 'A problem occurred in exporting Settings.', 'jupiterx' ) );
			}
	...
	private function _export_settings_options( $data ) {
		...
		foreach ( $option_keys as $option_key ) {
			$data['options']['extra'][ $option_key ] = get_option( $option_key );
		}
	...
	private function import_content() {
		try {
			...
			if ( ! file_exists( $file ) ) {
				throw new Exception(
					sprintf( __( 'A required file (%s) is missing in the selected zip file.', 'jupiterx' ), $file_name )
				);
			}
			...
			if ( is_wp_error( $data ) ) {
				throw new Exception(
					sprintf( __( 'Error in parsing %s.', 'jupiterx' ), $file_name )
				);
			}
	...
	private function import_widgets() {
		try {
			...
			if ( ! $this->import_widget_data( $data ) ) {
				throw new Exception( __( 'A problem occurred in importing Widgets.', 'jupiterx' ) );
			}
	...
	private function import_settings() {
		try {
			...
			if ( ! file_exists( $this->_get_import_package_dir_path( $file_name ) ) ) {
				throw new Exception(
					sprintf( __( '%s is missing in the selected zip file.', 'jupiterx' ), $file_name )
				);
			}
