...
class JupiterX_Control_Panel_Install_Template {
	...
	public function __construct( $system_test_env = false, $ajax_mode = true ) {
		...
		$this->setApiURL( 'http://artbees.net/api/v2/' );
	...
	public function installRequiredPlugins( $template_name ) {
		...
		try {
			$plugins = $this->getSettingsData( $template_name );

			if ( is_array( $plugins['options']['jupiterx_support_plugins'] ) == false || count( $plugins['options']['jupiterx_support_plugins'] ) <= 0 ) {
				throw new Exception( 'Plugin set have wrong structure' );
			}

			$plugin_manager = new JupiterX_Control_Panel_Plugin_Manager();
			$missed_plugins = [];

			foreach( $plugins['options']['jupiterx_support_plugins'] as $plugin ) {
				$plugin_basename = $plugin_manager->get_plugin_basename_by_slug( $plugin );
				if ( is_wp_error( validate_plugin( $plugin_basename ) ) ) {
					$missed_plugins[] = $plugin;
				}
			}

			if ( ! current_user_can( 'install_plugins' ) && ! empty( $missed_plugins ) ) {
				$missed_plugins_html = '<strong>' . implode( '</strong><strong>', $missed_plugins ) . '</strong>';

				$message = sprintf(
					esc_html__( 'You are using a sub site and you are not allowed to install a new plugin.
					Please install required plugins that listed below via your %s Network Admin > Plugins %s or contact your Network Admin.
					Required plugins for this template: %s %s %s', 'jupiterx' ),
					'<strong>',
					'</strong>',
			...
			$pm_response = $plugin_manager->install_batch( $plugins['options']['jupiterx_support_plugins'] );
			if ( ! $pm_response ) {
				$pm_response = $plugin_manager->get_response_message();
				throw new Exception( $pm_response );
			}

			$message = '{param} plugins are installed.';
			$this->message( array( $message, count( $plugins['options']['jupiterx_support_plugins'] ) ), true );
			return true;
		} catch ( Exception $e ) {
			$this->message( $e->getMessage(), false );
			return false;
	...
	public function import_plugins_content( $template_name ) {
		...
		try {
			...
			$this->message( __( 'Data of plugins have imported.', 'jupiterx'), true );
	...
	public function import_theme_content_sse() {
		try {
			...
			$importer = new JupiterX_Importer( $options );
	...
	public function import_settings( $template_name ) {
		try {
			...
			if ( 'array' != gettype( $data ) ) {
				throw new Exception(
					sprintf( __( 'Error importing settings! Please check that you uploaded (%s) a settings export file.', 'jupiterx' ), $file_name )
				);
			}
			if ( ! isset( $data['template'] ) || ! isset( $data['mods'] ) ) {
				throw new Exception(
					sprintf( __( 'Error importing settings! template Please check that you uploaded (%s) a settings export file.', 'jupiterx' ), $file_name )
				);
			}
	...
	public function resetWordpressDatabase( $tables = array(), $exclude_tables = array(), $install_needed = false ) {
		...
		if ( $install_needed ) {
			...
			if ( $jupiterx_temp_installed ) {
				...
				// Delete plugins.
				$plugins = $this->getSettingsData( $jupiterx_temp_installed );
				if ( $plugins ) {
					$JupiterX_Control_Panel_Plugin_Manager = new JupiterX_Control_Panel_Plugin_Manager( false, false );
					if(isset($plugins['options']['jupiterx_support_plugins'])) {
						foreach ( $plugins['options']['jupiterx_support_plugins'] as $required_plugin ) {
							if ( ! $JupiterX_Control_Panel_Plugin_Manager->deactivate( $JupiterX_Control_Panel_Plugin_Manager->get_plugin_basename_by_slug( $required_plugin ) ) ) {
	...
	public function importLayerSliderContent( $content_path ) {
		global $wpdb;
		$plugin  = new JupiterX_Control_Panel_Plugin_Manager();
