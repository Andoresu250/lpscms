...
class JupiterX_Control_Panel_System_Status {
	...
	public function ajax_handler() {
		...
		if ( ! $type ) {
			wp_send_json_error( esc_html__( 'Type param is missing.', 'jupiterx' ) );
		}

		$this->$type();

		wp_send_json_error(
			sprintf( esc_html__( 'Type param (%s) is not valid.', 'jupiterx' ), $type )
		);
	}
	...
	private function http_requests() {
		...
		if ( $blocked && 0 === sizeof( $hosts ) ) {
			wp_send_json_error( esc_html__( 'HTTP requests have been blocked by the WP_HTTP_BLOCK_EXTERNAL constant, with no allowed hosts.', 'jupiterx' ) );
		}

		if ( $blocked && 0 < sizeof( $hosts ) ) {
			wp_send_json_error(
				sprintf(
					esc_html__( 'HTTP requests have been blocked by the WP_HTTP_BLOCK_EXTERNAL constant, with some hosts whitelisted: %s.', 'jupiterx' ),
					implode( ',', $hosts )
				)
			);
		}
