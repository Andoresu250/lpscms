...
final class JupiterX_Setup_Wizard {
	...
	private function enqueue_scripts() {
		...
		wp_register_script( 'jupiterx-common', JUPITERX_ADMIN_ASSETS_URL . 'js/common' . JUPITERX_MIN_JS . '.js', [ 'wp-util' ], JUPITERX_VERSION, true );
		wp_enqueue_script( 'jupiterx-setup-wizard', JUPITERX_ADMIN_URL . 'setup-wizard/assets/js/setup-wizard' . JUPITERX_MIN_JS . '.js', [ 'jquery', 'wp-util', 'jupiterx-bootstrap', 'jupiterx-modal', 'imagesloaded', 'jupiterx-owl-carousel', 'jupiterx-common' ], JUPITERX_VERSION, true );
	...
	public function admin_notice() {
		...
		<div class="notice notice-info jupiterx-setup-wizard-message-box">
			...
			<p>
				<a href="<?php echo $this->get_url(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" class="button button-primary"><?php esc_html_e( 'Run Setup Wizard', 'jupiterx' ); ?></a>
			...
			<div class="jupiterx-nav owl-carousel">
				<?php foreach ( $this->pages_list as $id => $title ) { ?>
					<div class="jupiterx-nav-item">
						<span class="h2"><?php echo $title; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></span>
					</div>
				<?php } ?>
			</div>
			...
			<div class="jupiterx-template">
				<div class="jupiterx-card">
					...
					<div class="jupiterx-card-body">
						...
						<# } else { #>
							<a class="btn btn-primary mr-2 jupiterx-cp-template-item-btn jupiterx-upgrade-modal-trigger" href="#" data-upgrade-link="<?php echo esc_url( jupiterx_upgrade_link( 'templates' ) ); ?>"><?php esc_html_e( 'Import', 'jupiterx' ); ?></a>
						<# } #>
	...
	public function is_notice_hidden() {
		if ( ! current_user_can( 'manage_options' ) || ! empty( get_option( 'jupiterx_template_installed_id' ) ) ) {
			return true;
		}
