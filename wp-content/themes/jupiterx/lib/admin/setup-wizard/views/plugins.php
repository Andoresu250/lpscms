...
<div class="jupiterx-form">
	...
	<div class="jupiterx-plugins-list">
		<?php foreach ( $theme_plugins as $plugin ) { ?>
			...
			<?php $pro = 'true' === $plugin->pro ? true : false; ?>
			<?php $extra_class = $pro && ! jupiterx_is_pro() ? 'jupiterx-upgrade-modal-trigger' : ''; ?>
			<div class="custom-control custom-checkbox <?php echo esc_attr( $extra_class ); ?>">
				<input type="checkbox" class="custom-control-input" name="jupiterx-plugins" value="<?php echo $plugin->slug; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>" id="jupiterx-plugin-<?php echo esc_attr( $plugin->slug ); ?>" <?php echo $required ? 'disabled="disabled" checked="checked"' : ''; ?>>
