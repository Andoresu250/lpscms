...
function jupiterx_subtitle() {
	...
	jupiterx_open_markup_e(
		...
		echo $subtitle; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped -- Echoes HTML output.
...
function jupiterx_post_meta() {
	 ...
	 */
	if ( apply_filters( 'jupiterx_pre_post_meta', ! in_array( get_post_type(), [ 'post', 'portfolio' ], true ) ) ) {
		return;
	}
...
function jupiterx_post_tags() {
	...
	if ( ! $tags || ! jupiterx_get_option( 'jupiterx_post_tags', 'global', true ) ) {
		return;
	}
...
function jupiterx_post_social_share() {
	...
	if ( ! in_array( $post_type, [ 'post', 'portfolio' ], true ) ) {
		return;
	}
...
function jupiterx_post_navigation() {
	 ...
	 * @param bool $pre True to short-circuit, False to let the function run.
	 */
	if ( apply_filters( 'jupiterx_pre_post_navigation', ! is_singular( [ 'post', 'portfolio' ] ) ) ) {
		return;
	}
